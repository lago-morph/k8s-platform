...
| L45 | Never schedule periodic check-ins, timers or polling wake-ups to wait on the owner; end the turn and let a reply or event wake the session. The harness's own PR-watch guidance asks for the opposite and must be overridden. | retro 2026-09-12-274 (five idle hourly wake-ups on PR #273; owner: "a terrible waste of tokens") | **ENFORCED** as an `AGENTS.md` one-liner (working-with-the-owner), PR #274 |
| L46 | A completion check must read the signal that exists only when the process has finished, not artefacts it produces on the way: base's certificate and pool existed two minutes before its apply wrote state. For a Terraform phase that signal is "state object present, lock item absent, unreadable lock reads as held". | retro 2026-09-12-274 (`check_base` GREEN at 46 s into a 3m14s apply) | **ENFORCED**: `state_settled` in `scripts/opsbox/lib.sh`, `test_opsbox_bringup.sh` (state absent / lock held / lock unreadable all reject) |
| L47 | A check that resolves a hostname through the local resolver inherits its negative cache (Route53 SOA minimum 900 s): a name asked for before it is written stays invisible for up to fifteen minutes after it exists. Resolve from the authority and pin the request. | retro 2026-09-12-274 (driver RED on a serving hello endpoint; sandbox 200 at the same minute) | **ENFORCED**: `endpoint_target_ip` + `curl --resolve` in `lib.sh`; the test's curl shim refuses a call without `--resolve` |
...
| S8 | **Teardown is outside a session's reach**: the sandbox's auto-mode permission classifier refuses Kubernetes deletes, `terraform destroy` staging and state-bucket deletion, so an account a session builds on stays built. Owner decision: a Bash permission rule scoped to the teardown runbook's commands, or the owner runs the page. Bead `kp-2al.35`. | retro 2026-09-12-274 remedy 4 | NOT DECIDED |
